import Order from "../../../src/project/domain/entity/Order";
import crypto from "crypto";
import Product from "../../../src/project/domain/entity/Product";
import CurrencyTable from "../../../src/project/domain/entity/CurrencyTable";
...
test("Não pode adicionar item com quantidade menor ou igual a zero", async function () {
    const uuid = crypto.randomUUID();
    const cpf = "407.302.170-27";
    const order = new Order(uuid, cpf);
...
test("Deve criar um pedido com e gerar o código", async function () {
    const uuid = crypto.randomUUID();
    const cpf = "407.302.170-27";
